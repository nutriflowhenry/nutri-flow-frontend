...
const CardPostAdmin: React.FC<PostCardProps> = ({ post, handlePostStatus }) => {
    return (
        <div className={`flex flex-col md:flex-row shadow-lg rounded-lg mx-auto my-5 max-w-md md:max-w-2xl transition-shadow duration-300 hover:shadow-xl sm:pl-6 ${post.status !== "approved" ? "bg-gray-200" : "bg-white"} min-h-full h-auto`}>
        
            <div className="flex justify-center items-center md:justify-start md:items-start pt-4 pl-4">
                <ProfilePicture post={post} />
            </div>

        <div className="flex-1 py-6 pt-3 flex flex-col justify-between pr-6 h-auto min-h-full">
            ...
            <div className="mt-4 flex items-center">
                ...
                <button onClick={() => handlePostStatus(post.id, post.status)} className={`{mx-2 p-0.5 px-3 flex items-center text-white rounded-xl hover:shadow-md ${post.status === "approved" ? "bg-[#ed303083]" : "bg-[#38c44f88]"}`}>
                    <FontAwesomeIcon icon={post.status === "approved" ? faBan : faCheck} className={`text-1xl mr-2`} />
